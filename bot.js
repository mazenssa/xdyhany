...
client.on('message', async message => {
  ...
  if(message.content.startsWith(prefix + "mute")) {
    ...
    let mention = message.mentions.members.first();//kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
    if(!mention) return  message.channel.send('').then(msg => { //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
      msg.delete(3500);
      message.delete(3500);
    });
    ...
    if(mention.id === message.author.id) return message.channel.send('**You Cannot give mute to your self :x:**').then(msg => {
      msg.delete(3500);
      message.delete(3500); //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
    });
   
    if(mention.hasPermission('ADMINISTRATOR')) return message.channel.send(`**You can't give mute for a staff member. :x:**`); //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
    ...
    .addField('**Server**',[ message.guild.name ]) //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
    ...
    if(!role) try {
      message.guild.createRole({
        name: "Muted",
        permissions: 0 //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
      }).then(r => {
        message.guild.channels.forEach(c => {
          c.overwritePermissions(r , {
            SEND_MESSAGES: false, //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
            READ_MESSAGES_HISTORY: false,
            ADD_REACTIONS: false
          });
        });
      }); //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
    } catch(e) {
      console.log(e.stack);
    }
    mention.addRole(role).then(() => {
      ...
      mention.setMute(true); //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
    });
    setTimeout(() => {
      if(duration === 0) return;
      mention.setMute(false);
      mention.removeRole(role)
    },duration * 60000); //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
  }
});
...
let command = message.content.split(" ")[0];
     command = command.slice(prefix.length);
    let args = message.content.split(" ").slice(1);  //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
if(command === `unmute`) {2
...
if(!message.guild.member(client.user).hasPermission("MUTE_MEMBERS")) return message.reply("**I donot Have Permission Mute_Members**").then(msg => msg.delete(6000))
  ...
  if(!role || !kinggamer.roles.has(role.id)) return message.channel.sendMessage(`**:information_source:${mention.user.username} áÞÏ Êã Ýß ÇáãíæÊ Úäå ãÓÈÞÇ**`)
 
  await kinggamer.removeRole(role) //kinggamer ÍÞæÞ ÇáÝÇ ßæÏÒ æ
...
if(!message.member.hasPermission('MANAGE_MESSAGES')) return message.reply('**__You dont have Permission__**');
      ...
      client.on('message', message => {
        ...
        .setURL(`https://discordapp.com/api/oauth2/authorize?client_id=535468009100410883&permissions=2146958839&scope=bot`)      
     message.channel.sendEmbed(embed);
       }
   });
...
client.on('message', message => {
 // ÇáÈÑíÝßÓ
if(message.content.startsWith(prefix +"server")){ // ÇáÇãÑ
...
client.on('message',async msg => {
  ...
  if(!msg.guild.member(msg.author).hasPermissions('MANAGE_CHANNELS')) return msg.reply('? **áÇ ãÚß ÑÊÈå**');
...
client.on('message', msg => {
  if(msg.content === '?support')
  msg.channel.send('https://discord.gg/cPSjvf')
});
...
message.author.send(`
**.                             ? ????????? __General__ ????????? ?
????count | ÚÑÖ ÚÏÏ ÃÚÖÇÁ ÇáÓíÑÝÑ
????ping | ÚÑÖ ÓÑÚå ÇáÈæÊ
????avatar | ÚÑÖ ÕæÑÊß Ãæ ÕæÑå Ãí ÇÍÏ ÊãäÔäå
????image | áÚÑÖ ÕæÑå ÇáÓíÑÝÑ
????server | ãÚáæãÇÊ Úä ÇáÓíÑÝÑ
????help | ÚÑÖ åÐå ÇáÞÇÆãå
????bot | ãÚáæãÇÊ Úä ÇáÈæÊ
                              ? ????????? __Staff__ ????????? ?
????ban | ÍÙÑ ÔÎÕ ãä ÇáÓíÑÝÑ
????kick | ØÑÏ ÃÍÏ ãä ÃÚÖÇÁ ÇáÓíÑÝÑ
????mute | ÅÓßÇÊ ÔÎÕ ãä ÃÚÖÇÁ ÇáÓíÑÝÑ
????unmute | ÑÝÚ ÇáÅÓßÇÊ Úä ÇáÚÖæ
????banlist | ÚÑÖ ÚÏÏ ÇáäÇÓ ÇáãÍÙæÑå ãä ÇáÓíÑÝÑ
????mc | ÊÞÝíá ÇáÔÇÊ
????umc | ÝÊÍ ÇáÔÇÊ
????hide | ÅÎÝÇÁ ÇáÑæã
????show | ÅÙåÇÑ ÇáÑæã
????setstats | (ÅäÔÇÁ 3 ÑæãÇÊ ÈãÚáæãÇÊ Úä (ÇáÊÇÑíÎ , ÇáæÞÊ , ÚÏÏ ÇáäÇÓ ÇáãæÌæÏå Ýí ÇáÑæã ÇáÕæÊí
???-bc | ÑÓÇáå ÌãÇÚíå áßá ÇáäÇÓ Ýí ÇáÓíÑÝÑ
????clear | ãÓÍ ÇáÔÇÊ
????nickall | ÅÚØÇÁ ÌãíÚ ÇáäÇÓ Ýí ÇáÓíÑÝÑ ÅÓã ãÓÊÚÇÑ
                              ? ????????? __Information__ ????????? ?
????support | ÚÑÖ ÓíÑÝÑ ÇáÓÈæÑÊ Ýí ÍÇáå ãæÇÌåÉ Çí ãÔßáå
????inv / invite | áÏÚæå ÇáÈæÊ**
`);
    }
});
  ...
  message.channel.sendEmbed(embed);
    ...
    client.on('guildMemberAdd', member => {
        ...
        client.on('guildMemberRemove', member => {
            ...
            .setDescription(`ãÚ ÇáÓáÇãå ÔÑÝÊäÇ ?:skin-tone-1: ?? `)
...
client.on('message', msg => {
  if (msg.content === 'ÇáÓáÇã Úáíßã') {
    msg.reply('æÚáíßã ÇáÓáÇã ãäæÑ ÇáÓíÑÝÑ?');
  }
});
